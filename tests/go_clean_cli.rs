...
#[test]
fn go_module_cache_dry_run_does_not_execute_go() -> Result<(), Box<dyn std::error::Error>> {
    let temp = TempDir::new()?;
    let module_cache = temp.path().join("go").join("pkg").join("mod");
    let download = module_cache.join("cache").join("download");
    std::fs::create_dir_all(&download)?;
...
#[test]
fn go_module_cache_plan_replay_uses_go_clean_modcache() -> Result<(), Box<dyn std::error::Error>> {
    let temp = TempDir::new()?;
    let module_cache = temp.path().join("go").join("pkg").join("mod");
    let download = module_cache.join("cache").join("download");
    std::fs::create_dir_all(&download)?;
    ...
    assert!(
        !fake_go_output.exists(),
        "writing a dry-run plan must not execute go clean"
    );
    ...
    replay
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("mode: permanent"))
        .stdout(predicate::str::contains("Cleaned: 1 candidates"));
