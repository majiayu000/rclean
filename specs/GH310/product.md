...
helpers、assertions、fixtures、timeout 和 mutex 语义全部不变。拆分并由仓库固定的 rustfmt
规范化后，生产文件为 308 行、测试文件为 229 行，两者都回到仓库 200–400 行的典型范围。
...
2. **B-002** `src/clean/deletion/tests.rs` 必须等于基线父文件第 309–545 行逐行去掉四个前导空格
   后，再由当前 toolchain 的 `rustfmt --edition 2021` 规范化的结果；最终测试文件恰好 229 行。
...
- `cargo fmt` 会因移除一层缩进而压缩四处表达式；这四处变化必须完全来自同版 rustfmt。exact
  relocation proof 必须把同一份 dedented baseline 经 `rustfmt --emit stdout --edition 2021` 后再与
  child 比较，且在 fmt 后仍为空 diff。
