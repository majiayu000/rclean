...
交错运行，每个 revision 至少 15 个 warmed samples；报告 range/median。接受条件为 after
median 至少快 15%。三个既有 benchmark shapes 与 marker-heavy shape after point estimate
均不得超过 baseline 1.10x。不得在 CI/test 中写 wall-clock assertion。
...
- **Benchmark noise:** use interleaved same-session medians and structural probe-count evidence.
