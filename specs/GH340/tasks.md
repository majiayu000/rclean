...
- [ ] `SP340-T1` | Owner: `scan-walker` | Done when: the exact 339-line parent prefix remains and the private child module is declared | Verify: prefix hash/diff + exact declaration + 340-line count
- [ ] `SP340-T2` | Owner: `scan-walker-tests` | Done when: the four-test body moves through only the fixed dedent/Rust 2024 normalization and whole-file rollback is exact | Verify: raw/dedented/formatted hashes + child/rollback diffs + focused stable/MSRV tests
- [ ] `SP340-T3` | Owner: `verification` | Done when: scope, production/trust invariants, and all local/remote gates pass | Verify: scope/full/VibeGuard/CI/review/PR gates

## SP340-T1 — Externalize the module boundary

- Dependencies: merged Spec; latest main; unchanged baseline hashes in `tech.md`
- Covers: B-001, B-004, B-005
- Change:
  - keep `src/scan/walker.rs:1-339` byte-identical;
...
- Done when: prefix hash/diff is exact, line 340 is `mod tests;`, and the parent is 340 lines.
- Verify: fixed prefix proof, declaration search, public/scope audit, line count.

## SP340-T2 — Move the Rustfmt-normalized test body

- Dependencies: SP340-T1
- Covers: B-002, B-003, B-004, B-005
- Change:
  - add `src/scan/walker/tests.rs` from baseline lines 341-457;
  - remove one four-space nesting indent, then apply Rust 2024 Rustfmt;
  - preserve the four tests, imports, mutex/catch-unwind fixtures, panic/error strings, candidate data, and assertions.
- Done when: all three source hashes match, child diff and whole-file rollback are empty, no adjacent file changes,
  and stable/exact Rust 1.95.0 focused runs pass 4/4.
- Verify: fixed forward/rollback proof, inventory/visibility search, focused stable/MSRV tests.

## SP340-T3 — Prove merge readiness

- Dependencies: SP340-T1 and SP340-T2
...
- Change exactly `src/scan/walker.rs` and add `src/scan/walker/tests.rs`; do not touch production or other tests.
- Preserve the 339-line prefix byte-for-byte; use only the pinned dedent plus Rust 2024 formatting for the body.
- Preserve all four poison-path tests and their panic/error contracts; no traversal or trust-model behavior changes.
