...
Its body is the exact existing `src/doctor.rs:74-341` sequence plus a final `entries` return. It owns the anchor
helper imports currently used by that sequence. It must not reorder, consolidate, data-drive, or otherwise rewrite
...
- all rule IDs, paths, reason strings, status variants, and anchor-helper arguments are unchanged;
